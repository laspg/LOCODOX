function sx = nanstd(x, flag, max_miss)
...
%    NANSTD(X).
%  
%    NANSTD(X,0,max_miss) normalizes by N and produces the second moment of the
%    available data about their mean.  NANSTD(X,1) is the same as
%    NANSTD(X). max_miss indicate the maximum admissible fraction of missing
%    values
%
%    See also STD, NANMEAN.
  ...
  if nargin < 2, flag = 1; max_miss = 0.6; end        % default: normalize by nobs-1
  if nargin < 3, max_miss = 0.6; end                  % default: frac = 0.6
